...
const Tables = () => {
    ...
    const [tablePerPage, setTablePerPage] = useState(pageSizes[0]);
    ...
    const getTables = async (value) => {
        ...
        await fetch(`http://localhost:8000/tables?number=${searchValue}&limit=${pageSizeValue}`, {
            method: 'GET',
            headers: {
    ...
    const deleteTable = async (id) => {
        if (window.confirm('Bạn có muốn xóa không?')) {
            const databaseRef = ref(getDatabase(), `tables/${id}`); // Thay '/path/to/data' bằng đường dẫn bạn muốn xóa
    ...
    const totalPage = Math.ceil(totalTable / tablePerPage);
    const lastTableIndex = (page + 1) * tablePerPage;
    const firstTableindex = lastTableIndex - tablePerPage;
    const currentTables = tables.slice(firstTableindex, lastTableIndex);

    const onClickNext = (next) => setPage(page + next > totalPage ? page : page + next);
    const onClickPrev = (next) => setPage(page - next < 0 ? 0 : page - next);
    const onChangeSearch = () => (getTables());
                    ...
                    <CCardBody>
                        <CRow className="">
                            ...
                            <CCol xs={12} sm={6} className="text-end">
                                <button type="button" className="btn" onClick={getTables}><CIcon icon={cilSearch} size="xl" /></button>
                                <button type="button" className="btn" onClick={() => { navigate(`/table/add`) }}><CIcon icon={cilPlus} size="xl" /></button>
                            </CCol>
                        </CRow>                       
                            ...
                            <table className="table table-striped ">
                                <thead>
                                    <tr>
                                        ...
                                        <th style={{textAlign: 'center'}}>Số bàn</th>
                                        <th style={{textAlign: 'center'}}>Số chỗ ngồi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {currentTables?.map((table, i) => (
                                        <tr key={i}>
                                            <td style={{textAlign: 'center'}}>{table.id}</td>
                                            <td style={{textAlign: 'center'}}>{table.number}</td>
                                            <td style={{textAlign: 'center'}}>{table.seat}</td>
                                            <td>
                                                <div className="btn-group">
                                                    <Link
                                                        to={{ pathname: `/table/${table.id}` }}
                                                        state={{ id: table.id, number: table.number, seat: table.seat }}>
                                            ...
                                            <td>
                                                <div className="btn-group">
                                                    <button type="button" onClick={() => { deleteTable(table.id) }} className="btn">
